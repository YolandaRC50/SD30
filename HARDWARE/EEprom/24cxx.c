...
{				  
	...
	if(EE_TYPE>AT24C16)
	{
		EEP_I2C_Send_Byte(0XA0);	   //发送写命令
		EEP_I2C_Wait_Ack();
		EEP_I2C_Send_Byte(ReadAddr>>8);//发送高地址	    
	}else EEP_I2C_Send_Byte(0XA0+((ReadAddr/256)<<1));   //发送器件地址0XA0,写数据 	   
	EEP_I2C_Wait_Ack(); 
    EEP_I2C_Send_Byte(ReadAddr%256);   //发送低地址
	EEP_I2C_Wait_Ack();	    
	EEP_I2C_Start();  	 	   
	EEP_I2C_Send_Byte(0XA1);           //进入接收模式			   
	EEP_I2C_Wait_Ack();	 
