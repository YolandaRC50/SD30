...
void f_copy(char* orign_path,char* aim_path)
{
	FIL orgin_file, aim_file;
	FRESULT fr;
	UINT br,bw;
	BYTE buffer[4096];
	
	fr = f_open(&orgin_file,orign_path,FA_READ);
	fr = f_open(&aim_file,aim_path,FA_CREATE_ALWAYS | FA_WRITE);
	for(;;)
	{
		fr = f_read(&orgin_file,buffer,sizeof(buffer),&br);
		if(fr||br==0)break;
		fr = f_write(&aim_file,buffer,sizeof(buffer),&bw);
		if(fr||bw<br)break;
	}
...
u8 mf_copy(u8 *psrc,u8 *pdst,u8 fwmode)
{
 ...
 fbuf=(u8*)mymalloc(SRAMIN,512);
   if(fsrc==NULL||fdst==NULL||fbuf==NULL)res=100;//??????fatfs
 else
 {   
  ...
  if(res==0)//????????
  {
    while(res==0)//????
   {
    res=f_read(fsrc,fbuf,512,(UINT*)&br); //????512??
    if(res||br==0)break;
