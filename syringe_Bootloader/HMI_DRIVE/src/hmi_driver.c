...
uint8_t RSRT(uint8_t vbWaitRe)
{
  ...
  while(cmd_Updata_Mark!=0xA5)
	...
	if(cmd_Updata_Mark!=0xA5 || TIM_Counter>=3000)
	{
		return 0;
	}
	else
...
uint8_t SendFiledData(uint8_t *pData,uint16_t nDataLen,uint8_t vbWaitRe)
{
	//int TIM_Counter=0;
	cmd_Updata_Mark=0;
	...
	return RSRT(vbWaitRe);
} 
uint8_t SendUint16(uint16_t value,uint8_t vbWaitRe)
{
	TX_16(value);
	return RSRT(vbWaitRe);
}
...
uint8_t BeginUpdateHMI(void)
{
	uint8_t cmd[] = {0xEE,0xF0,0x04,0xBF,0xFF,0xFC,0xFF,0xFF};
	return SendFiledData(cmd,8,1);
//	
}
uint8_t  EndUpdateHMI(void)
{
	
	uint8_t cmd[] = {0xF1}; 
	return SendFiledData(cmd,1,1);
	
}
uint8_t StartDownLoad(void)
{
	
	uint8_t cmd[] = {0xEE,0xF0,0xFF,0xFC,0xFF,0xFF};
	
	return SendFiledData(cmd,6,1);
}